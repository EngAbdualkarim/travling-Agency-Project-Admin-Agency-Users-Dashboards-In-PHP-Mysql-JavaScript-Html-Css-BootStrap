...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strpos($email, '@') === false) {
        echo "<script>alert('Invalid email format. Must contain @');window.location.href = 'login_agency.php';</script>";
        exit;
    }
    if (strlen($password) < 6) {
        echo "<script>alert('Password must be at least 6 characters');window.location.href = 'login_agency.php';</script>";
        exit;
    }

    $checkAgencyQuery = "SELECT agency_id, password FROM travel_agency WHERE email = ?";
    $checkAgencyStmt = $conn->prepare($checkAgencyQuery);
    $checkAgencyStmt->bind_param("s", $email);
    $checkAgencyStmt->execute();
    $result = $checkAgencyStmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            echo "<script>alert('Invalid password');window.location.href = 'login_agency.php';</script>";
        }
    } else {
        echo "<script>alert('Agency not found');window.location.href = 'login_agency.php';</script>";
    }

    $checkAgencyStmt->close();
    $conn->close();
    exit;
}
...
<body>
    <header>
        ...
        <div class="boxs mt-5 p-4">
            <h1 class="text-center mb-4" style="border-bottom: 2px solid #3498db; padding-bottom: 10px; font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 32px; color: #333;">Login As Agency</h1>
            <form action="login_agency.php" method="post">
